...
class Search extends Controller
{
    ...
    public function customerSearch(){
        ...
        $result = Vendor::selectRaw(' concat('.$this->getConcatCondition("id").','.$this->getConcatCondition("name").','.$this->getConcatCondition("email").','.$this->getConcatCondition("phone").','.$this->getConcatCondition("ic").') as label,`id` as value, `company_id`, `user_id`, `name`, `email`, `tax_number`, `phone`, `address`, `website`, `currency_code`, `enabled`, `created_at`, `updated_at`, `deleted_at`, `reference`, `ic`, `customer_id`')->whereRaw(" enabled = 1 and (id like '%".$searchKeyword."%' or name like '%".$searchKeyword."%' or email like '%".$searchKeyword."%' or phone like '%".$searchKeyword."%' or ic like '%".$searchKeyword."%' ) ")->get();
